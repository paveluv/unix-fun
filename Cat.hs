...
getOptions = do
  ...
  where
    ...
    options =
      [ Option "n" ["number"] (NoArg N) "number all output lines"
      ]
...
main = do
  ...
  let filepaths = if null filepaths' then ["-"] else filepaths'

--  let filters = [filterN']
--  let cat = \s -> putStr $ foldl (\s f -> f s) s filters
  let cat = if elem N flags then catNumbered else L.putStr
