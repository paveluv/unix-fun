...
printLong opts dirpath items = do
  ...
  where
    ...
    fileModeStr fullPath status = do
      hasXattr <-
        hasXattr
          (if flag_L opts
             then []
    ...
    addRow (total, rows) (path, status) =
      let fullPath = dirpath </> path
       in do ownerName <- getUserName opts (fileOwner status)
             groupName <- getGroupName opts (fileGroup status)
             currentTime <- getCurrentTime
             timeStr <- getTimeStr currentTime status
             fileModeStr <- fileModeStr fullPath status
             lastCol <-
               if isSymbolicLink status
                 then do
                   linkTarget <- getSymbolicLinkTarget fullPath
